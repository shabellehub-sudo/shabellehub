...
import { classifyWithAI } from './classifyWithAI.js';
...
// Known Limitation (B6 — Cross-Product Attribution Gap, deferred 2026-09-02):
// diffSnapshots() diffs the entire page text with no source-section
// attribution. A source page that covers multiple products (e.g. a
// Wikipedia comparison table listing ChatGPT, Gemini, Claude, etc. side
// by side) can produce a hunk that is real content, correctly NOT
// filtered as noise below, but that actually belongs to a different
// product than the one being monitored -- e.g. another product's
// "Active"/"Discontinued" status bleeding into this tool's diff.
//
// No heuristic proximity fix (checking whether the tool's name appears
// within N characters of the hunk) is applied here on purpose: an
// arbitrary character threshold risks false negatives, and silently
// classifying a real change as noise is worse than the current
// false-positive risk -- a suppressed real change is unrecoverable,
// while a false positive only costs a manual Dismiss click in
// /admin/monitoring. The correct fix is product-specific section
// extraction in fetchSnapshot.js, upstream of this file, so diffing
// only ever sees text that belongs to the monitored tool. Deferred
// until this pattern recurs often enough in production to justify it
// (tracked as "B6 Phase 2").
function isNoiseHunk(value, { before = '', after = '' } = {}) {
  ...
  // Fix (Track B #1): a run of digits/brackets/whitespace with no letters
  // (e.g. "148 ] [ 149", "[ 151 ] ") is a citation-reference-number cluster,
  // not real content. diffWords groups adjacent renumbered citations
  // ("[148][149]" -> "[149][150]") into one multi-token hunk that the
  // single-token pure-numeric check above doesn't catch. Square brackets
  // never appear in a real price/plan value, so this is safe unconditionally
  // (no currency-context exception needed, unlike the pure-numeric case).
