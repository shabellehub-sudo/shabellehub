...
import { getMonitoringUrl } from './sources';
...
export async function runMonitoringBatch(toolSlugs) {
  ...
  for (const slug of toolSlugs) {
    ...
    const sourceUrl = getMonitoringUrl(tool);
    if (!sourceUrl) {
    ...
    const snap = await fetchSnapshot(slug, sourceUrl);
    ...
    if (snap.outcome === 'fetch_failed' || snap.outcome === 'fetch_incomplete') {
      ...
      results.push({ slug, outcome: snap.outcome });
      await sleep(DELAY_BETWEEN_TOOLS_MS);
      continue;
    }
