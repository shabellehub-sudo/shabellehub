...
export async function runMonitoringBatch(toolSlugs) {
  ...
  for (const rawSlug of toolSlugs) {
   try {
    ...
    if (snap.outcome !== 'fetched') {
      ...
      if (withinBudget) {
        ...
        if (grounded && grounded.signal) {
          // Fix (Track B B4.2): the ai_search branch used to ignore
          // createPendingChange()'s return value and always log
          // 'ai_search_grounded' (success), even when the insert was
          // silently skipped as a duplicate or failed outright. Mirrors
          // the main diff path's skipped/!ok/ok handling (verified in
          // B4.3) so a DB failure here is never misreported as success.
          ...
          if (pendingResult.skipped) {
            await logAudit(db, runId, slug, sourceUrl, 'duplicate_suppressed', 'AI-search change matches an existing pending change within the last 24h');
            results.push({ slug, outcome: 'duplicate_suppressed', category: grounded.category });
          } else if (!pendingResult.ok) {
            await logAudit(db, runId, slug, sourceUrl, 'db_insert_failed', pendingResult.error || 'tool_changes insert failed (ai_search)');
            results.push({ slug, outcome: 'db_insert_failed', category: grounded.category, error: pendingResult.error });
          } else {
    ...
    // Fix (Track B B5.4): per-tool isolation. Any unexpected exception
    // for one tool (network throw, malformed response, a bug in a
    // helper, etc.) must never abort the batch for every other tool.
    // Logged clearly for audit/debugging, then the loop continues.
    const errorMessage = err?.message || String(err);
    try {
      await logAudit(db, runId, rawSlug, null, 'tool_exception', errorMessage);
    } catch (auditErr) {
      // Even audit logging can fail (e.g. DB unreachable) -- never let
      // that escape either, or the isolation guarantee breaks anyway.
      console.error(`[monitoring] Failed to log audit for ${rawSlug}:`, auditErr?.message || auditErr);
    }
    results.push({ slug: rawSlug, outcome: 'tool_exception', error: errorMessage });
    await sleep(DELAY_BETWEEN_TOOLS_MS);
   }
  }
