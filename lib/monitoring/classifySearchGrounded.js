...
// Daily safety cap — Gemini 3.x grounding gives 5,000 free prompts/month
// shared across the family. At ~100 tools total this is nowhere close to
// the limit, but the cap exists so a bug can never run up an unexpected
// bill.
const DAILY_CAP = 50;
...
Focus specifically on: pricing changes, new model or version releases, new feature launches, or whether the product has been discontinued or shut down.

Respond with ONLY a JSON object, no markdown formatting, no code fences, no extra text. Use this exact shape:
{"found": true or false, "category": "status" or "model_update" or "pricing" or "features" or "unknown", "summary": "one sentence, max 25 words", "signal": true or false}
...
// Checks how many AI-search-grounded calls have already run today, using
// the existing monitoring_audit_log table — no new table needed.
...
export async function classifySearchGrounded(toolName) {
  ...
  try {
    ...
    return {
      ...
      confidence: 'low', // always low — single point-in-time, no diff history
