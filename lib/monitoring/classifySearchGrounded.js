...
export async function classifySearchGrounded(toolName) {
  ...
  try {
    ...
    // Fix (Track B B3.1): extract a real citation URL from Gemini's
    // grounding metadata when present, instead of reusing the failed
    // direct-fetch URL as "evidence" -- that URL was never actually
    // read by anything, human or AI, so presenting it as the source
    // is misleading. Falls back to null (no link) when no citation is
    // available; a missing link is more honest than a wrong one.
    const groundingChunks = json?.candidates?.[0]?.groundingMetadata?.groundingChunks || [];
    const citationUrl = groundingChunks.find((c) => c?.web?.uri)?.web?.uri || null;

    return {
      category: parsed.category || 'unknown',
      confidence: 'low',
      signal: !!parsed.signal,
      // Review-safety: mark this clearly as an unverified single-point
      // AI claim (no diff/history behind it) directly in the text shown
      // to admins, since dashboard UI is out of scope for this fix.
      summary: `[AI-generated, unverified] ${(parsed.summary || '').slice(0, 180)}`.slice(0, 200),
      source: 'ai_search',
      evidenceUrl: citationUrl,
    };
  } catch (err) {
    return null;
  } finally {
