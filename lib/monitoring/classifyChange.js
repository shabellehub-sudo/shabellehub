...
export function classifyChange(diffText) {
  // Fix (Track B B4.1): only scan ADDED (+) lines for category keywords.
  // Scanning removed (-) lines too let boilerplate/nav-menu text that
  // disappears during an unrelated content change (e.g. a "Pricing" nav
  // link vanishing when a page's layout changes) falsely trigger a
  // category match. Removed content is stale by definition and must
  // never drive classification on its own.
  ...
  const hasCurrency = CURRENCY_PATTERN.test(addedText);
