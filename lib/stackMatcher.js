...
export function getComplementaryStack(primaryTool, allTools = []) {
  const result = getSmartStackForTool(primaryTool, allTools);
  return {
    primary: primaryTool,
    asset: result?.creationTool || null,
    distribution: result?.distributionTool || null,
  };
}
