...
const { makeFirestoreShim } = require('./firestoreShim');
...
// `profiles`. Returns { error, uid, role, db } — db is a Firestore-shim
// wrapped client (see firestoreShim.js) so existing .collection()/.doc()
...
async function authenticate(req) {
  ...
  return { error: null, uid, role: profile.role, db: makeFirestoreShim(client) };
}
