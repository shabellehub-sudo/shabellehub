    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ borderCollapse: 'collapse', width: '100%' }}>
          <thead>
            <tr>
              <th style={{ padding: '4px', fontSize: 11, color: '#6b82a8', textAlign: 'left', width: 90 }}>Label</th>
              {columns.map((col, i) => (
            ...
            {rows.map((row, ri) => (
              <tr key={ri} style={{ background: row.highlight ? 'rgba(20,255,244,0.03)' : 'transparent' }}>
                <td style={{ padding: '4px' }}>
                  <input style={inp} value={row.label || ''} onChange={e => setRow(ri, 'label', e.target.value)} placeholder="Row label" />
                </td>
