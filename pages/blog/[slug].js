...
// ── Markdown renderer (FIXED: handles fenced code blocks + inline code) ──────
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (fenceMatch) {
      if (!inCodeBlock) {
        ...
        html.push(
          `<pre style="background:#0a0e16;border:1px solid #1a2d4a;border-radius:10px;padding:16px 18px;overflow-x:auto;-webkit-overflow-scrolling:touch;max-width:100%;margin:20px 0"><code style="font-family:Menlo,Consolas,monospace;font-size:13.5px;line-height:1.6;color:#c5d3ea;white-space:pre;overflow-wrap:normal">${codeHtml}</code></pre>`
        );
        codeBuffer = [];
      }
      continue;
    ...
    html.push(`<p style="color:#9fb3d4;font-size:15px;line-height:1.8;margin-bottom:20px">${inlineFormat(escapeHtml(line))}</p>`);
  }
