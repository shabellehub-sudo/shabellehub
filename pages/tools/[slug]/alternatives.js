...
export async function getStaticProps({ params }) {
  ...
  // Re-check against live data, not just the static build snapshot used in
  // getStaticPaths -- a tool that has dropped below the threshold since
  // the last static build should 404 on the next revalidation instead of
  // continuing to silently serve a now-thin page.
  if (alternatives.length < 3) {
    return { notFound: true };
  }

  return { props: { tool, alternatives, source }, revalidate: 3600 };
}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
          {alternatives.map((t) => (
            <Link key={t.id} href={`/tools/${t.slug}`} style={{ textDecoration: 'none' }}>
              <div style={{ background: '#0f1829', border: '1px solid #1a2d4a', borderRadius: 14, padding: 20 }}>
